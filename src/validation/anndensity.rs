...
fn get_block_stats(blocnum : usize, blockout : &Vec<usize>) -> f64 {
...
pub fn density_analysis<F,D, N>(graph : &Graph<N, f64, Undirected>, embedded : &Embedded<F>, 
            hnsw_opt : Option<Hnsw<F,DistPtr<F,f64>>>) -> Result<Vec<(usize, Vec<usize>)>, anyhow::Error>
    where F : Copy+Clone+Send+Sync ,
          D : Distance<F> , 
          N : std::marker::Copy  {
    //
    ...
    for (i, v) in &res_analysis {
        log::info!("\n density_analysis for densest block : {i}, block size : {:?}", decomposition.get_nbpoints_in_block(*i).unwrap());
        let frac_out = get_block_stats(*i, v);
...
mod tests {
    ...
    fn ann_check_density_miserables() {
        ...
        //
        let block_analysis = density_analysis::<usize, DistPtr<usize, f64>, u32>(&graph, &embedded, None);
