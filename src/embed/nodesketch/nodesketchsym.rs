...
impl NodeSketch {
    ...
    pub(crate) fn dump_row_iteration(&self, noderank: usize) {
        ...
        println!(
            "previous state  {:?}",
            self.previous_sketches[noderank].read()
        );
    ...
    fn sketch_slamatrix(&mut self, parallel: bool) {
        ...
        let treat_row = |row: usize| {
            ...
            log::trace!(
                " sketch_slamatrix sketch row i : {} , sketch : {:?}",
                row,
                sketch
            );
    ...
    fn parallel_iteration(&mut self) {
        ...
        (0..self.csrmat.rows())
            .into_par_iter()
            .for_each(|row| self.treat_row(&row));
        // transfer sketches into previous sketches
        for i in 0..self.get_nb_nodes() {
            let mut row_write = self.previous_sketches[i].write();
            for j in 0..self.get_sketch_size() {
...
mod tests {
    ...
    fn test_nodesketch_lesmiserables() {
        ...
        log::debug!(
            "node (11,27)  =  rank({},{})",
            embed_res.get_node_rank(11).unwrap(),
            embed_res.get_node_rank(27).unwrap()
        );
        log::debug!(
            "distance between nodes 11 and 27 : {}, weight in file {} ",
            dist_11_27,
            31
        );
        let dist_11_33 = embed_res.get_node_distance(11, 33);
        log::debug!(
            "distance between nodes 11 and 33 {} , weight in file : {}",
            dist_11_33,
            1
        );
        ...
        log::debug!(
            "\n\n row {:?} sketch {:?} ",
            rank,
            embedded.get_embedded().row(rank)
        );
        ...
        log::debug!(
            "embedded distance between nodes 21 and 23 : {:3.e}",
            dist_21_23
        );
        let rank = 48;
        log::trace!(
        ...
        log::debug!(
            "distance between nodes n1 : {} n2 : {}, dist : {:3.e}",
            node_of_rank_26,
            node_of_rank_27,
            dist
        );
