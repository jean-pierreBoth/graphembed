...
use crate::embed::gkernel::{idmap::*, pgraph::*};
...
pub fn load_labels(dir: &String) -> anyhow::Result<(HashMap<u32, Vec<u8>>, IndexMap<u8, u8>)> {
    ...
    if fileres.is_err() {
        log::error!(
            "read_ppi_data : reload could not open file {:?}",
            filepath.as_os_str()
        );
        println!(
    ...
    for (nb_record, result) in rdr.records().enumerate() {
        ...
        if record.len() != nb_fields {
            log::error!(
                "record num : {}, record : {:?}, record length : {}",
                nb_record,
                &record,
                record.len()
            );
...
#[allow(unused)]
pub fn read_ppi_directed_data(
    dir: String,
) -> anyhow::Result<(
    Graph<PpiNode, PpiEdge, petgraph::Directed, DefaultIx>,
    IdMap<u8, u8>,
)> {
    //
    let delim = b' ';
    ...
    for (nb_record, result) in rdr.records().enumerate() {
        ...
        if let Ok(idx) = field.parse::<u32>() {
            ...
            if !nodeset.contains_key(&node1) {
                ...
                // node already in graph, we must retrieve its index
        ...
        if let Ok(idx) = field.parse::<u32>() {
            ...
            log::error!("error decoding node 2 of record  {}", nb_record + 1);
            return Err(anyhow!(
                "error decoding node 2 of record  {}",
                nb_record + 1
            ));
        }
...
#[allow(unused)]
pub fn read_ppi_undirected_data(
    dir: String,
) -> anyhow::Result<(
    Graph<PpiNode, PpiEdge, petgraph::Undirected, DefaultIx>,
    IdMap<u8, u8>,
)> {
    //
    let delim = b' ';
    ...
    log::info!(
        "nb self loops : {}, nb bidirectional edges {}",
        nb_self_loops,
        nb_double_dir
    );
    log::info!("nb nodes = {}", graph.raw_nodes().len());
