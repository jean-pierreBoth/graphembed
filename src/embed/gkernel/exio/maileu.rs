...
#[allow(unused)]
pub fn read_maileu_data(
    dir: String,
) -> anyhow::Result<(
    Graph<EuNode, EuEdge, petgraph::Directed, DefaultIx>,
    IdMap<u8, u8>,
)> {
    //
    let delim = b' ';
    ...
    if fileres.is_err() {
        log::error!(
            "read_maileu_data : reload could not open file {:?}",
            filepath.as_os_str()
        );
        println!(
    ...
    for result in rdr.records() {
        ...
        if let Ok(idx) = field.parse::<u32>() {
            ...
            if !nodeset.contains_key(&node1) {
                ...
                if log::log_enabled!(log::Level::Debug) && label == 0 {
        ...
        log::debug!(
            " index1 {} , index2 {}, gnode1 {:?} gnode2 {:?} edge count {}",
            node1,
            node2,
            gnode1.unwrap(),
            gnode2.unwrap(),
